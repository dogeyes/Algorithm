 ...
 */
public class BSTMultiST<Key extends Comparable<Key>, Value> {
    private Node root;
    private class Node
    {
        Key key;
    ...
    public Iterable<Value> getAll(Key key)
    {
        ...
        getAll(root, key, queue);
        return queue;
    }
    private void getAll(Node x, Key key, Queue<Value> queue)
    {
        if(x == null)
            return;
        ...
        if(comp == 0)
        {
            ...
            getAll(x.left, key, queue);
            getAll(x.right, key, queue);
        }
        else if(comp < 0)
        {
            getAll(x.left, key, queue);
    ...
    {
        BSTMultiST<Integer, Integer> bst = new BSTMultiST<Integer, Integer>();
